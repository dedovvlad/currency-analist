...
def test_response_not_auth(requests_mock, response_without_auth):
    ...
    """
    requests_mock.get(
        URL_API_BOT_DATA.format(BOT_API_KEY), json=response_without_auth, status_code=401
    )
    get_chat = chat._get_group_id_from_bot()
    assert get_chat == 401
...
def test_response_bed_request(requests_mock, response_bed_request):
    ...
    """
    requests_mock.get(
        URL_API_BOT_DATA.format(BOT_API_KEY), json=response_bed_request, status_code=404
    )
    get_chat = chat._get_group_id_from_bot()
    assert get_chat == 404
